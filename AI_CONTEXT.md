...
- **آخر فحوص مكتملة قبل تحديث GitHub:** 168 اختبار Vitest، TypeScript، بناء الإنتاج، ميزانية الحزمة، وفحص هاتف بعرض 390px.
...
- **قرار Visual Repair Loop (مراجعة فقط، غير منفذ):** الاقتراح صحيح في اتجاهه، لكن الإصلاح الحالي `restore-readable-background` لا يصلح إلا حالة عنوان الإعلان ذات الخلفية العامة. لا يجوز تطبيقه تلقائياً على السعر أو التذييل أو الشعار؛ هذه لا تملك إصلاحات محلية مضمونة بعد. يجب أن تبدأ v1 بحلقة يضغطها المستخدم للعنوان فقط: `repair → render → Pixel Truth → DesignContract → verify`، مع Snapshot قابل للتراجع من `TemplateSettings` لا من `DesignDocument` وحده، لأن عقد التصميم الحالي هندسي ولا يحمل لون الخلفية. لا تدّع اختبار Canvas كاملاً داخل Vitest/JSDOM؛ يوثق الفحص البصري في المتصفح منفصلاً حتى تتوفر بنية اختبار متصفح حقيقية.
