...
const labels: Record<ArtworkLayerKey, string> = { header: 'بانر العنوان', footer: 'بانر التذييل', logo: 'الشعار الدائري' };
...
export default function ArtworkPositionEditor({ settings, onChange }: ArtworkPositionEditorProps) {
  const visibleLayers = artworkLayerKeys.filter(layer => layer === 'header' ? settings.showHeaderArtwork && settings.headerArtwork : layer === 'footer' ? settings.showFooterArtwork && settings.footerArtwork : settings.showStoreLogo && settings.storeLogoArtwork);
  const [selectedLayer, setSelectedLayer] = useState<ArtworkLayerKey>(visibleLayers[0] || 'header');
  ...
  const layerImage = (layer: ArtworkLayerKey) => layer === 'header' ? settings.headerArtwork : layer === 'footer' ? settings.footerArtwork : settings.storeLogoArtwork;
  ...
  const active = getArtworkTransform(settings, selectedLayer);
  ...
  return <section className="mt-5 rounded-2xl border border-primary/15 bg-primary/[0.03] p-4" dir="rtl">
    ...
    <div className="mt-3 flex flex-wrap gap-2">{visibleLayers.map(layer => <button key={layer} type="button" onClick={() => setSelectedLayer(layer)} className={`rounded-xl px-3 py-2 text-xs font-black ${selectedLayer === layer ? 'bg-primary text-white' : 'bg-white text-primary shadow-sm'}`}>{labels[layer]}</button>)}</div>
    <div ref={previewRef} onPointerMove={onPointerMove} onPointerUp={() => { drag.current = null; }} onPointerCancel={() => { drag.current = null; }} className="relative mx-auto mt-4 w-full max-w-[280px] overflow-hidden rounded-2xl border-2 border-dashed border-primary/25 bg-white shadow-inner" style={{ aspectRatio, touchAction: 'none' }}>
      <div className="absolute inset-x-[17%] top-[22%] bottom-[19%] rounded-xl border border-dashed border-stone-300 bg-stone-50/70"><span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 whitespace-nowrap text-[10px] font-bold text-stone-400">منطقة الملابس</span></div>
    ...
    <div className="mt-4 rounded-xl bg-white p-3"><div className="flex items-center justify-between gap-2"><span className="text-xs font-black text-foreground">طريقة ملاءمة {labels[selectedLayer]}</span><button type="button" onClick={() => updateLayer(selectedLayer, getDefaultArtworkTransform(settings.size, selectedLayer))} className="inline-flex items-center gap-1 text-xs font-black text-primary"><RotateCcw size={14} /> إعادة الضبط</button></div><div className="mt-2 grid grid-cols-3 gap-2">{fitOptions.map(option => <button key={option.value} type="button" onClick={() => updateLayer(selectedLayer, { ...active, fit: option.value })} className={`rounded-lg px-2 py-2 text-[11px] font-black ${active.fit === option.value ? 'bg-primary text-white' : 'bg-secondary text-primary'}`}>{option.label}</button>)}</div></div>
  </section>;
}
