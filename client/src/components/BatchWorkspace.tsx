...
import type { AdDetails, BatchAdItem, TemplateSettings } from '@shared/types';
...
export default function BatchWorkspace({ details, template, onDetailsChange, onBack, previewItems = [] }: BatchWorkspaceProps) {
  ...
  const persist = async (nextItems: BatchAdItem[], localRemoval = useLocalRemoval) => {
  ...
  const processBatch = async (retryFailed = false) => {
    ...
    for (const item of candidates) {
      ...
      try {
        ...
        updateItem(item.id, { status: 'success', outputUrl, usedLocalRemoval: useLocalRemoval });
      } catch (error) {
