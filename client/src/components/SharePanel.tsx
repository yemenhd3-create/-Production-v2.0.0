...
export default function SharePanel({ onDownload, onShare, onWhatsApp, onEdit, onClear }: SharePanelProps) {
  return (
    <section className="grid grid-cols-2 gap-3" dir="rtl" aria-label="خيارات الإعلان النهائي">
      <p className="col-span-2 rounded-2xl bg-secondary/55 px-4 py-3 text-center text-xs leading-5 text-muted-foreground">احفظ الإعلان أو شاركه الآن. سيبقى قابلاً للتعديل حتى تختار مسح الجلسة.</p>
      ...
      <button type="button" onClick={onClear} className="col-span-2 inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-3 text-sm font-black text-red-700 transition active:scale-[0.98]"><Eraser size={18} /> مسح جلسة الإعلان (الصورة والبيانات) وبدء جديد</button>
    </section>
  );
}
