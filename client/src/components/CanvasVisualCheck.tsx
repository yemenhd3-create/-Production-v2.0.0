...
export default function CanvasVisualCheck() {
  ...
  return <main className="min-h-screen bg-[#fffdf6] p-5 text-center" dir="rtl"><h1 className="text-xl font-black text-[#2A2865]">فحص هندسة قوالب الإعلان</h1><p className="mt-2 text-sm leading-6 text-slate-600">كل مقاس يعرض حالتين: ملابس بخلفية ملونة وقصاصة شخص PNG شفافة. يجب أن تبقى منطقة البطل هي الأكبر دائماً.</p>{error && <p className="mt-5 rounded-xl bg-red-50 p-4 text-red-700">{error}</p>}<section className="mx-auto mt-6 max-w-5xl space-y-6">{sizes.map(size => <article key={size.value} className="rounded-2xl bg-white p-3 text-right shadow-sm"><h2 className="mb-3 text-sm font-black text-[#2A2865]">{size.label}</h2><div className="grid gap-3 sm:grid-cols-2"><Preview label="ملابس بخلفية ملونة" url={previews[`${size.value}-raw`]} /><Preview label="شخص شفاف (Try-On)" url={previews[`${size.value}-person`]} /></div></article>)}</section></main>;
}
