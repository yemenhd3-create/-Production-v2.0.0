...
export default function CanvasVisualCheck() {
  ...
  useEffect(() => {
    ...
    Promise.all(sizes.flatMap(({ value }) => {
      ...
      return [
        renderAd(DEFAULT_AD_DETAILS, { ...DEFAULT_TEMPLATE_SETTINGS, size: value, productBackdrop: 'spotlight', productShadow: 'grounded' }, transparentGarment, { ...dimensions, visualMode: 'garment' }).then(url => ({ key: `${value}-studio`, url })),
  ...
  return <main className="min-h-screen bg-[#fffdf6] p-5 text-center" dir="rtl"><h1 className="text-xl font-black text-[#2A2865]">فحص هندسة قوالب الإعلان</h1><p className="mt-2 text-sm leading-6 text-slate-600">كل مقاس يعرض غرفة الملابس البسيطة ثم القوالب التفصيلية. يجب أن تبقى القطعة هي أكبر عنصر مرئي.</p>{error && <p className="mt-5 rounded-xl bg-red-50 p-4 text-red-700">{error}</p>}<section className="mx-auto mt-6 max-w-6xl space-y-6">{sizes.map(size => <article key={size.value} className="rounded-2xl bg-white p-3 text-right shadow-sm"><h2 className="mb-3 text-sm font-black text-[#2A2865]">{size.label}</h2><div className="grid gap-3 sm:grid-cols-4"><Preview label="غرفة الملابس" url={previews[`${size.value}-studio`]} /><Preview label="ملابس بخلفية ملونة" url={previews[`${size.value}-raw`]} /><Preview label="شخص شفاف (Try-On)" url={previews[`${size.value}-person`]} /><Preview label="شعار وتذييل كامل" url={previews[`${size.value}-layers`]} /></div></article>)}</section></main>;
}
